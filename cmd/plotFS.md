...
# 📂 plotFS.ps1 – Dokumentacja Techniczna

**Wersja:** 1.25.0

Zaawansowany wizualizator struktury plików dla PowerShell, wyposażony w **Logikę Stref (Zone Logic)** oraz **Tunelowanie (Tunneling)**. Pozwala na precyzyjne filtrowanie, ignorowanie folderów systemowych przy jednoczesnym "wyciąganiu" z nich konkretnych plików (np. `.exe` z katalogu `target/`).

---

## ⚠️ Ważna Uwaga Logiczna

> **UWAGA:** Ze względu na specyfikę implementacji silnika Regex:
> 1. Aby uzyskać **wszystkie pliki oraz puste foldery**, należy użyć wzorca:
> `-pathPatterns "*", "!temp/", ...`
> 2. Wzorzec rekurencyjny `-pathPatterns "**/*"` może w pewnych konfiguracjach pomijać puste foldery w głównym katalogu.
> 
> 
> Zaleca się stosowanie `*` jako domyślnego wzorca startowego.

---

## 🚀 Szybki Start

Pobierz skrypt i zapisz go jako `plot-fs.ps1`.

### Przykładowe użycie (Rust Project):

```powershell
.\plot-fs.ps1 -pathPatterns "*", "!target/", "!**/.git/", "target/release/*.exe" -sortStrategy B -sortDirFirst
...
1. `*` – Pokazuje wszystko.
2. `!target/` – Ukrywa folder `target` i jego zawartość.
3. `target/release/*.exe` – **Tuneluje się** do środka wykluczonego `target`, aby pokazać pliki `.exe`.
4. `-sortStrategy B` – Sortuje pliki wg rozszerzeń.
5. `-sortDirFirst` – Foldery wyświetlane przed plikami.
...
| --- | --- | --- | --- |
| **`pathLocation`** | `String` | Ścieżka do katalogu, który chcesz wizualizować. | `.` (Bieżący) |
| **`pathPatterns`** | `String[]` | Lista wzorców włączających (`*`) i wykluczających (`!dir/`). | `*` |
| **`sortStrategy`** | `String` | Algorytm sortowania plików: $\\$ **A** (Alfabetyczny), **B** (Po rozszerzeniu), **C** (Sufiksowy). | `A` |
| **`sortExtDepth`** | `Int` | Głębokość analizy sufiksów (tylko dla strategii `C`). | `4` |
| **`sortDirFirst`** | `Switch` | Jeśli podana, foldery będą wyświetlane nad plikami. | `$false` |
| **`sortPriority`** | `String[]` | Lista plików, które zawsze mają być na samej górze. | *Lista domyślna* |

---

## 🧩 Logika Filtrów i Wzorców (Glob Patterns)

Silnik skryptu zamienia wzorce Glob na wyrażenia regularne (Regex). Poniższa tabela przedstawia obsługiwane wzorce.

| Wzorzec | Zakres / Opis |
| --- | --- |
| **Baza** |  |
| `**/*` | Wszystko (pliki + foldery, rekurencyjnie). |
| `**/` | Wszystkie katalogi (bez plików). |
| `**/*.*` | Wszystkie pliki z rozszerzeniami (bez folderów). |
| `!**/` | Wyklucz wszystkie katalogi (pokaż tylko pliki). |
| **Ścieżki i Zakotwiczenia** |  |
| `**/src/*` | Pliki bezpośrednio w `src/` *(gdziekolwiek w drzewie)*. |
| `/**/src/*` | Pliki bezpośrednio w `src/` *(gdziekolwiek wewnątrz roota)*. |
| `*/src/*` | Pliki bezpośrednio w `src/` *(znajdującym się na drugim poziomie)*. |
| `/*/src/*` | Pliki bezpośrednio w `src/` *(znajdującym się na drugim poziomie od roota)*. |
| `src/*` | Pliki bezpośrednio w `src/` *(w miejscu uruchomienia lub zależnie od kotwicy)*. |
| `/src/*` | Pliki bezpośrednio w `src/` *(wyłącznie w głównym katalogu projektu)*. |
| `src/**` | Wszystko wewnątrz `src/` (pliki i foldery). |
| `/src/**` | Wszystko wewnątrz `src/` (zakotwiczone w root). |
| **Złożone Grupowanie** |  |
| `**/{gen,src}/**/test/*.{ts,rs}` | Pliki `.ts`/`.rs` leżące bezpośrednio w folderze `test` (będącym w `gen` lub `src`). |
| `**/{gen,src}/**/test/*/*.{ts,rs}` | Pliki `.ts`/`.rs` leżące dokładnie jeden poziom głębiej niż folder `test`. |
| `**/{gen,src}/**/test/**/*.{ts,rs}` | Pliki `.ts`/`.rs` leżące na dowolnym poziomie wewnątrz folderu `test`. |
| **Rozszerzenia** |  |
| `*.ts` | Pliki `.ts` (tylko bieżący folder, bez podfolderów). |
| `*.{ts,rs}` | Pliki `.ts` oraz `.rs` (tylko bieżący folder). |
| `*.?s` | Pliki z dwuznakowym rozszerzeniem kończącym się na `s` (np. `.ts`, `.rs`, `.js`, `.css` - nie pasuje, bo 3 znaki). |
| `src/**/*.ts` | Wszystkie pliki `.ts` wewnątrz `./src` i jego podfolderach. |
| `**/*.ts` | Wszystkie pliki `.ts` wszędzie. |
| `**/gen/**/*.ts` | Wszystkie pliki `.ts` wewnątrz dowolnego folderu `gen`. |
| `**/gen/**/*.?s` | Rozszerzenia 2-znakowe kończące się na `s` wewnątrz `gen`. |
| `**/gen/**/*.{ts,rs}` | Wszystkie `.ts` i `.rs` wewnątrz `gen`. |
| `**/gen/**/{*.ts,*.rs}` | Alternatywny zapis powyższego. |
| **Zakresy Znakowe** |  |
| `lib_[a-z].rs` | Dopasuje `lib_a.rs`, `lib_b.rs`, ale **nie** `lib_1.rs`. |
| `file[123].ts` | Dopasuje `file1.ts`, `file2.ts`, `file3.ts`. |
| `*[0-9]*.?s` | Pliki zawierające cyfrę w nazwie i dwuznakowe rozszerzenie na `s`. |
| `*[a-z]*.?s` | Pliki zawierające literę w nazwie i dwuznakowe rozszerzenie na `s`. |
| `[A-Z]` | Dowolna wielka litera. |
| `[a-zA-Z]` | Dowolna litera (mała lub wielka). |
| `[a-f0-9]` | Znaki szesnastkowe (hex). |
| `[0-5]` | Tylko cyfry od 0 do 5. |
| `[!0-9]*` | Nazwa zaczynająca się od znaku, który **nie** jest cyfrą. |

---

## 📊 Strategie Sortowania (`-sortStrategy`)

Skrypt oferuje trzy tryby układania plików na liście:

1. **Strategia A (Domyślna)**
* **Logika:** `Priorytety` -> `Nazwa Alfabetycznie`.
* Najprostszy widok. Pliki specjalne (`Cargo.toml`) są na górze, reszta alfabetycznie.


2. **Strategia B (Rozszerzenia)**
* **Logika:** `Priorytety` -> `Rozszerzenie` -> `Nazwa`.
* Grupuje pliki typami. Wszystkie `.rs` będą obok siebie, wszystkie `.ts` obok siebie.


3. **Strategia C (Głębokość Sufiksów)**
* **Logika:** `Priorytety` -> `Segmenty Sufiksu` -> `Nazwa`.
* Dla plików o złożonych nazwach, np. `komponent.test.js` vs `komponent.js`.
* Parametr `-sortExtDepth` określa ile "kropek" od końca jest analizowanych.
...
Skrypt automatycznie przypisuje ikony do plików:

* 🦀 `.rs` (Rust)
* 🦕 `.ts` (TypeScript)
* 📦 `.exe` (Binarki)
* ⚙️  Konfiguracja (`.toml`, `.json`, `.gitignore`)
* 🔒  Lockfiles
* 🏗️  Skrypty budujące (`build.rs`)
* 🖼️  Media

---

## 💻 Kod Źródłowy (V1.25.0)

Poniżej znajduje się kompletny kod skryptu.
